const RealEstate = require('../db/models/realestate');
const { catchAsync } = require('../utils/api.utils');
const AppError = require('../errors/app.error');
// const factory = require('./handler.factory');

exports.createRealEstate =  catchAsync(async (req, res, next) => {
  res
    .status(201)
    .json({ status: true, data: '', message: 'Created successfully' });
});

exports.getAllRealEstates =  catchAsync(async (req, res, next) => {
  res
    .json({ status: true, data: '', message: 'Retrieved successfully' });
});

exports.getFilteredRealEstates =  catchAsync(async (req, res, next) => {
  res
    .json({ status: true, data: '', message: 'Retrieved successfully' });
});

exports.getRealEstate =  catchAsync(async (req, res, next) => {
  res
    .json({ status: true, data: '', message: 'Retrieved successfully' });
});

exports.updateRealEstate =  catchAsync(async (req, res, next) => {
  res
    .json({ status: true, data: '', message: 'Updated successfully' });
});

exports.archiveRealEstate = catchAsync(async (req, res, next) => {
